...
def is_valid(puzzle, guess, row, col):
    ...
    for r in range(row_start, row_start + 3):
        for c in range(col_start, col_start + 3):
            if puzzle[r][c] == guess:
                return False
    # if we get here, this checks pass
    return True
...
def solve_sudoku(puzzle):
    ...
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3, ....9
        ...
        if is_valid(puzzle, guess, row, col):
            # step 3.1: if this is a valid guess,  then place it at the spot on the puzzle!
            puzzle[row][col] = guess
            # now recurse using this puzzle!
            # step 4: then we recursively call our function
            if solve_sudoku(puzzle):
                return True
        
        # step 5: it is not valid OR if our guess does not solve the puzzle, then we need to 
